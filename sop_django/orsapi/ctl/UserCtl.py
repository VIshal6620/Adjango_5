...
class UserCtl(BaseCtl):
    ...
    def request_to_form(self, requestForm):
        self.form["id"] = requestForm.get("id",0)  # None if not provided
    ...
    def save(self, request, params={}):
        try:
            ...
            if self.input_validation():
                return JsonResponse({
                    "success": False,
                    "result": {"inputerror": self.form["inputError"]}
                })

            user_id = int(self.form["id"])
            login_id = self.form["loginId"]

            # ===== UPDATE =====
            if user_id > 0:
                if User.objects.exclude(id=user_id).filter(loginId=login_id).exists():
                    return JsonResponse({
                        "success": False,
            ...
            # ===== ADD =====
                ...
                if User.objects.filter(loginId=login_id).exists():
                    return JsonResponse({
                        "success": False,
                        "result": {"message": "Login Id already exists"}
                    })
